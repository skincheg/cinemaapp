...
struct MovieAbout: View {
    ...
    var body: some View {
        ZStack {
//            if isShowingVideo {
                CustomVideo(movieData: movieAbout)
//            }
...
struct CustomVideo : UIViewControllerRepresentable {
    var movieData : MovieAboutViewModel
    
    func makeUIViewController(context: Context) -> AVPlayerViewController {
        let controller = AVPlayerViewController()
//        if let url = URL(string: "http://cinema.areas.su/up/video/\(movieData.episodes?.first?.preview ?? "")".addingPercentEncoding(withAllowedCharacters: .urlQueryAllowed) ?? "") {
//            let player = AVPlayer(url: url)
//            controller.player = player
//        }
        
        let player = AVPlayer(url: URL(string: "https://i.imgur.com/8etC5iX.mp4")!)
        controller.player = player
